...
_LOC_PATS = [
    re.compile(r"(?:[가-힣A-Za-z0-9]+)?아파트\s*\d+\s*동?\s*(?:앞|후|옆|근처|인근)?\s*(?:도로|출입구|입구|주차장|정문|후문)?"),
    re.compile(r"[가-힣A-Za-z0-9]+아파트(?:\s*\d+\s*동)?(?:\s*(?:앞|후|옆|근처|인근))?(?:\s*(?:도로|출입구|입구|주차장|정문|후문))?"),
    re.compile(r"[가-힣A-Za-z0-9]+(?:로|길)\s?\d*(?:-\d+)?"),
    re.compile(r"[가-힣A-Za-z0-9]+(?:초등학교|학교|공원|역|병원|사거리|교차로|공사장|마트|편의점|놀이터)(?:\s*인근)?"),
    re.compile(r"(?!(?:^|[\s])(우리|저희)(?:$|[\s]))[가-힣A-Za-z0-9]+(?:시|군|구|읍|면|동|리)"),
]

# === 보강 유틸 ===
_STOP_WORDS = {"우리", "저희"}
_SUFFIX_PAT = re.compile(
    r"(?:에서|에)\s*발생했습니다\.?$|(?:이|가)\s*필요합니다\.?$|입니다\.?$|합니다\.?$|요\.?$"
)

def _strip_eos(s: str) -> str:
    s = (s or "").strip()
    s = re.sub(_SUFFIX_PAT, "", s)
    s = re.sub(r"\s{2,}", " ", s)
    return s

def _nounify_fields(f: ComplaintFields) -> ComplaintFields:
    return ComplaintFields(
        위치=_strip_eos(f.위치),
        현상=_strip_eos(f.현상),
        문제점=_strip_eos(f.문제점),
        위험성=_strip_eos(f.위험성),
        요청사항=_strip_eos(f.요청사항),
    )
...
def _pick_location(text: str) -> str:
    ...
    if not cands:
        return ""
    cands = [v for v in cands if v not in _STOP_WORDS]
    cands.sort(key=len, reverse=True)
    chosen = _clean_loc(cands[0])[:80]
    return _strip_eos(chosen)
...
def _heuristic(text: str) -> ComplaintFields:
    t = _normalize(text)
    t = re.sub(r'^(우리|저희)\s+', '', t)

    sents = _sent_split_ko(t)
    first = sents[0] if sents else t

    위치     = _pick_location(t) or _first(_ADDR_PAT, t)
    문제점   = _first(_PROB_PAT, t)
    위험성   = _first(_RISK_PAT, t)
    요청사항 = _first(_REQ_PAT, t)
    현상     = first or ""

    joined_for_rules = f"{문제점} {t}"
    if not 위험성 and ("주정차" in joined_for_rules):
        위험성 = "보행자·차량 시야 가림 및 충돌 사고 위험"
    if not 요청사항 and ("주정차" in joined_for_rules):
        요청사항 = "단속 강화 및 안내 표지 설치 요청"
    if not 요청사항 and ("파손" in joined_for_rules):
        요청사항 = "긴급 보수 요청"
...
def _llm_fill_missing(base_text: str, locked: ComplaintFields) -> Optional[ComplaintFields]:
    """
    휴리스틱으로 채운 값은 '잠금'으로 두고, 비어있는 칸만 LLM이 보완.
    """
    sys = (
        "너는 한국어 민원서 '필드 추출기'다. "
    ...
    try:
        ...
        joined = f"{base_text} {merged.get('현상','')} {merged.get('문제점','')}"
        if (merged.get("위험성") in ("", "미상")) and ("주정차" in joined):
            merged["위험성"] = "보행자·차량 시야 가림 및 충돌 사고 위험"
        if (merged.get("요청사항") in ("", "미상")) and ("주정차" in joined):
            merged["요청사항"] = "단속 강화 및 안내 표지 설치 요청"

        return _nounify_fields(ComplaintFields(**merged))
    except Exception:
        return None
...
def run_extract(req: SummarizeRequest) -> ComplaintFields:
    """
    텍스트(+이미지 URL 힌트) → (휴리스틱) → 부족하면 LLM 보강 → nounify → tidy_fields
    """
    ...
    if _score(fields_rule) >= 6 or not _ollama_ok():
        return tidy_fields(_nounify_fields(fields_rule))

    fields_llm = _llm_fill_missing(joined, fields_rule)
    if fields_llm:
        return tidy_fields(_nounify_fields(fields_llm))
